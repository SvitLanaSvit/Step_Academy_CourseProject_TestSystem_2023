    ...
    if (isset($_GET['category'])) {
        ...
        if ($category == $_SESSION['category']) {
            if (!isset($_SESSION['randomQuestion'])) {
                ...
                //Check set answers or not, if set than return only that question on that not answers in session
            ...
            if ($totalQuestions == 0) {
                ...
                echo "<div class='alert alert-warning'>The list of questions on this category " . strtoupper($category) . " is empty!</div>";
            }
            ...
            $currentQuestionIndex = isset($_SESSION['currentQuestionIndex']) ? $_SESSION['currentQuestionIndex'] + 1 : 0;
            $questionIndex = $currentQuestionIndex;
            $correctAnswer = 0;
            if(!isset($_SESSION['correctAnswer']) && $totalQuestions != 0){
    ...
    ?>
                ...
                <div class="container">
                    <form method="POST" id="questionForm" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="correctAnswer" id="correctAnswer" value="<?= $_SESSION['correctAnswer'] ?>">
                        <div class="mb-3">
                            <div id="question">
            ...
            ?>
            <script>
                var userResponse = confirm('Do you want to continue your previous test?');
                if (userResponse) {
                    window.location.href = 'index.php?page=19&category=<?=$category?>';
                } else if(!userResponse) {
                    let xhr = new XMLHttpRequest();
                    xhr.open('GET', 'pages/tests/set_session_variable.php', true);
                    xhr.send();
